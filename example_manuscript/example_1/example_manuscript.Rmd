...
```{r}
df = read.csv("MARP_data.csv")
```

```{r}
mod1 = lm(wb_overall_mean ~ religiosity + age + gender + ses + education, data = df)
apa_lm = apa_print(mod1)
apa_table(apa_lm$table, caption = "Well-being regression table.")
```

The estimate for religiosity is `r apa_lm$estimate$religiosity`
...
knitr::include_graphics("Figure1A.png", dpi = 1200)
```
...
\noindent
```{r, fig.heigth = 1, fig.width = 12}
knitr::include_graphics("Figure1B.png", dpi = 1200)
```
