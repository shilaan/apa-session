...
I'll first give a very brief conceptual introduction to the what and why of reproducible manuscripts, and then we'll spend the bulk of our time on the practical aspects of writing fully reproducible manuscripts. 
...
On the slides, I'm sharing a link to a GitHub repo with example manuscripts that you can look at now, or look at later for future reference if you're inspired to give the things I'll discuss today a try in one of your future projects.  

This repo includes a link to the slides and instructions for installing R, RStudio, and RMarkdown, and for cloning the contents of the repository to your local computer. Once you've cloned the contents of this repo, you'll be able to follow along at your own pace and take a look at three fully reproducible manuscripts generated with R Markdown.  

During this talk, I'll really start with the basics, so if you've already had some prior exposure to R and RMarkdown then you may want to start with the online materials on GitHub instead.
...
Now you may have attended Mike’s conceptual talk on this topic last week, in which case you’ll see some overlap in the Workflow that I’m about to introduce and the personal experiences that Mike discussed last week. 
...
A question to keep in mind when we discuss this workflow is what kind of challenges or problems could arise, and maybe what kind of problems you've personally experienced while working in this fashion. Of course feel free to comment in the chat if you want to share your thoughts or horror stories. 
...
Time related to transcribing, copy pasting, formatting references, fixing the layout of your document after you've added another Table or Figure in Word, time related to tracking down mistakes or other sources of differences when you all of a sudden get a different result...
...
This is something that came through very clearly in Mike Frank's talk - the prevalence of statistical reporting errors in psychological science. 
...
Multiple inconsistent versions of code, data, or both
...
In the conceptual talk on reproducible manuscripts, Mike very clearly discussed the rationale behind reproducible manuscripts, with one of them being error detection and mitigation. To borrow Mike's words, a reproducible report is easier to audit, and because we avoid manual tasks like copy pasting, we reduce opportunities for error.  
...
Maya, Percy, and Mike also discussed this in their earlier talks in the lecture series: when you invest in reproducible research practices, there's a steep learning curve at first - but it is immensely time-saving in the long term. To again borrow Mike's words, It's easier to build on your prior work by re-using your code, and there's no need to start from scratch each time or remind yourself what you did previously. 
...
Like Maya said in her conceptual talk on reproducible data analysis - investing in reproducible practices doesn’t just benefit other people, it benefits your future self - it’s an investment in your future sanity.
...
A dynamic scientific report interleaves the written report with blocks of code used to conduct the analyses. The text is automatically formatted as a scientific paper in several potential styles. When the text is formatted, the code blocks are evaluated and their results are insterted in the text, or rendered as figures and tables. Dynamic document generation replaces the classical approach of using separate programs to write prose and conduct analyses, and then manually copy-pasting analysis results into the text. When revisions require major changes to the analyses, all results, figures and tables
are automatically updated. The flexibility in output formats also means that a manuscript can be rendered to PDF, to word, or even to a poster, website, or blog post.
...
If you do not submit to APA journals, no need for despair, because there are many other packages out there to help you write a fully reproducible report in the style of your choosing. If this applies to you, I'd suggest taking a look at the R package rticles, without an A. rticles includes templates for a wide variety of formats and journals, including those required by Elsevier, Frontiers, PLOS, PNAS, Sage, Springer, and Taylor & Francis.  
...
If I had more time, I would have discussed:
- Version control with Git and GitHub
- Reproducible software environments, with renv or groundhog or Docker.  

In the chat, we're posting links to two papers that can further help you on your journey to reproducible research with R, and that also discuss version control and reproducible software environments.
...
My solutions are definitely not perfect, but as always I think that some solution is better than no solution. To maximize uptake, I personally believe it's important to strike a balance between rigor and ease of use. 